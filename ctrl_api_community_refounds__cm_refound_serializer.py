...
from controllers.api.community.refounds.serializers.cm_refound_line_serializer import CmRefoundLineSerializer
from controllers.api.community.refounds.serializers.cm_refound_discountline_serializer import CmRefoundDiscountLineSerializer
from controllers.api.community.refounds.serializers.cm_refound_pointline_serializer import CmRefoundPointLineSerializer
from controllers.api.community.refounds.serializers.cm_refound_voucherline_serializer import CmRefoundVoucherLineSerializer
from controllers.api.community.orders.serializers.cm_cashcount_serializer import CmCashCountSerializer
from controllers.api.community.refounds.serializers.cm_refound_payment_serializer import CmRefoundPaymentSerializer
from controllers.api.community.refounds.serializers.cm_idlecommerce_serializer import CmIdlEcommerce
from controllers.api.community.refounds.serializers.cm_idlecommercedevoluciones_serializer import CmIdlEcommerceDevoluciones
from controllers.api.community.refounds.serializers.cm_refound_expensesline_serializer import CmRefoundExpensesLineSerializer


class CmRefoundsSerializer(DefaultSerializer):

    def get_data(self):
        if not self.control_tallas_devolucion():
            return False

        codigo = "WDC" + qsatype.FactoriaModulos.get("flfactppal").iface.cerosIzquierda(str(self.init_data["rma_id"]), 9)
        ...
        if self.init_data["status"] != "Complete" or "items_requested" in self.init_data:
            ...
            for line in self.init_data["items_refunded"]:
                ...
                line_data = CmRefoundLineSerializer().serialize(line)
            ...
            if "items_requested" in self.init_data:
                for linea in self.init_data["items_requested"]:
                    ...
                    line_data = CmRefoundLineSerializer().serialize(linea)
                    self.data["children"]["lines"].append(line_data)
    ...
    def crear_registros_descuentos(self, iva):
        ...
        linea_descuento = CmRefoundDiscountLineSerializer().serialize(new_init_data)
    ...
    def crear_registros_puntos(self, iva):
        ...
        linea_puntos = CmRefoundPointLineSerializer().serialize(new_init_data)
    ...
    def crear_registros_vales(self, iva):
        ...
        linea_vale = CmRefoundVoucherLineSerializer().serialize(new_init_data)
    ...
    def cerrar_devolucionweb(self, codigo):
        ...
        arqueo_web = CmCashCountSerializer().serialize(self.data)
    ...
    def crear_pagos_devolucionweb(self, arqueo_web, codigo):
        ...
        pago_web = CmRefoundPaymentSerializer().serialize(new_init_data)
    ...
    def crear_registros_ecommerce(self):
        ...
        idl_ecommerce_devolucion = CmIdlEcommerceDevoluciones().serialize(new_init_data)
        self.data["children"]["idl_ecommerce_devolucion"] = idl_ecommerce_devolucion

        excluir_idl = False
        if "items_requested" in self.init_data:
            idl_ecommerce = CmIdlEcommerce().serialize(new_init_data)
    ...
    def crear_registros_gastosenvio(self, iva):
        ...
        linea_gastosenvio = CmRefoundExpensesLineSerializer().serialize(new_init_data)
