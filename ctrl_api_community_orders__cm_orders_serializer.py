...
from controllers.api.community.orders.serializers.cm_shippingline_serializer import CmShippingLineSerializer
from controllers.api.community.orders.serializers.cm_expensesline_serializer import CmExpensesLineSerializer
from controllers.api.community.orders.serializers.cm_discountline_serializer import CmDiscountLineSerializer
from controllers.api.community.orders.serializers.cm_voucherline_serializer import CmVoucherLineSerializer
from controllers.api.community.orders.serializers.cm_payment_serializer import CmPaymentSerializer
from controllers.api.community.orders.serializers.cm_cashcount_serializer import CmCashCountSerializer
from controllers.api.community.orders.serializers.cm_idlecommerce_serializer import CmIdlEcommerce


class CmB2COrdersSerializer(DefaultSerializer):
    ...
    def get_data(self):
        ...
        if str(self.init_data["status"]) == "holded" or str(self.init_data["status"]) == "closed":
            ...
            for item in self.init_data["items"]:
                ...
                line_data = CmOrderLineSerializer().serialize(item)
            ...
            linea_envio = CmShippingLineSerializer().serialize(new_init_data)
            linea_gastos = CmExpensesLineSerializer().serialize(new_init_data)
            linea_descuento = CmDiscountLineSerializer().serialize(new_init_data)
            linea_vale = CmVoucherLineSerializer().serialize(new_init_data)
            arqueo_web = CmCashCountSerializer().serialize(self.data)
            new_data = self.data.copy()
            new_data.update({"idarqueo": arqueo_web["idtpv_arqueo"]})
            pago_web = CmPaymentSerializer().serialize(new_data)
