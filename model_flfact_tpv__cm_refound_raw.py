...
from models.flfact_tpv.objects.cm_refound_line_raw import CmRefoundLine
from models.flfact_tpv.objects.cm_refound_payment_raw import CmRefoundPayment
    ...
    def get_children_data(self):
        for item in self.data["children"]["lines"]:
            if item:
                ...
                self.children.append(CmRefoundLine(item))
        ...
        if "payments" in self.data["children"]:
            for item in self.data["children"]["payments"]:
                self.children.append(CmRefoundPayment(item))
