...
class CmB2CPointsProcess(DownloadSync):
    ...
    def __init__(self, driver, params=None):
        ...
        self.origin_field = "codtarjetapuntos"
        self.idlogs = ""
        self.small_sleep = 10
        self.large_sleep = 60
    ...
    def get_data(self):
        ...
        q.setWhere("website = 'community' AND not procesado AND (estadoprocesado IS NULL OR estadoprocesado = '') ORDER BY fechaalta, horaalta LIMIT 10")
    ...
    def after_sync(self):
        success_records = []
        error_records = [tarjeta["codtarjetapuntos"] for tarjeta in self.error_data]
        after_sync_error_records = []

        for tarjeta in self.success_data:
            try:
                qsatype.FLSqlQuery().execSql("UPDATE eg_logtarjetasweb SET procesado = true, fechaprocesado = CURRENT_DATE, horaprocesado = CURRENT_TIME, estadoprocesado = 'OK' WHERE idlog IN ({}) AND codtarjetapuntos = '{}'".format(self.idlogs, tarjeta["codtarjetapuntos"]))
                success_records.append(tarjeta["codtarjetapuntos"])
            except Exception as e:
                self.after_sync_error(tarjeta, e)
                after_sync_error_records.append(tarjeta["codtarjetapuntos"])

        for tarjeta in self.error_data:
            try:
                qsatype.FLSqlQuery().execSql("UPDATE eg_logtarjetasweb SET procesado = true, fechaprocesado = CURRENT_DATE, horaprocesado = CURRENT_TIME, estadoprocesado = 'ERROR' WHERE idlog IN ({}) AND codtarjetapuntos = '{}'".format(self.idlogs, tarjeta["codtarjetapuntos"]))
            except Exception as e:
                self.after_sync_error(tarjeta, e)
                after_sync_error_records.append(tarjeta["codtarjetapuntos"])
