...
# @class_declaration community_sync_upload #
from models.flsyncppal import flsyncppal_def as syncppal


class community_sync_upload(interna_upload):
    ...
    def start(pk, data):
        ...
        if "passwd" in data and data["passwd"] == syncppal.iface.get_param_sincro('apipass')['auth']:
            response = task_manager.task_executer("products_community_upload", data)
            if response:
...
class upload(community_sync_upload):
    pass
